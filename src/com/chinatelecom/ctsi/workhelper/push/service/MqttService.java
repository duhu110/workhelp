...
import com.chinatelecom.ctsi.workhelper.pushutil.MLog;
 ...
 * </p>
 */
public class MqttService extends IntentService implements MqttTraceHandler {
  ...
  void reconnect() {
	...
	if(connections.size()==0) {
		getClient(PushUtil.BROKER_URL, PushUtil.getClientId(getApplicationContext()), getApplicationInfo().packageName, null);
  ...
  public void subscribe(String clientHandle, String topic, int qos,
    ...
    connection.subscribe(topic, qos, invocationContext, activityToken);
  }
  ...
  public int onStartCommand(final Intent intent, final int flags, final int startId) {
	...
	MLog.log("MQTTSERVICE", "onStartCommand");
	PushUtil.addAlarmTask(getApplicationContext());
	if(intent!=null) {
...
//					Connection connection = Connections.getInstance(getApplicationContext())
//							.getConnection(PushUtil.getHandle(getApplicationContext()));
//					if(connection==null || connection.getClient()==null || !connection.getClient().isConnected()) {
//						 try {
//							connect(PushUtil.getHandle(getApplicationContext()),PushUtil.getOptions(),
//								      null, "1");
