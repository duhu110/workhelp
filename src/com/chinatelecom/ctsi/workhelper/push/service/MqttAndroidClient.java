...
public class MqttAndroidClient extends BroadcastReceiver implements
	...
	public enum Ack {
		 ...
		 * to {@link com.chinatelecom.enterprisecontact_qx.push.service.MqttAndroidClient} using {@link com.chinatelecom.enterprisecontact_qx.push.service.MqttAndroidClient#acknowledgeMessage(String)}
		 */
		MANUAL_ACK
	}
	...
	// the service
	////zwt private String clientHandle;
	...
	////zwt private String clientId;
	private MqttClientPersistence persistence = null;
	////zwt private MqttConnectOptions connectOptions;
	...
	public boolean isConnected() {
		if (mqttService != null)
			return mqttService.isConnected(getClientHandle());
		else 
			return false;
	}

	private String getClientHandle() {
		//return clientHandle;
		return mqttService.getClient(serverURI,getClientId(), myContext.getApplicationInfo().packageName,persistence);
	}
	...
	public String getClientId() {
		return PushUtil.getClientId(mqttService);
	}
	...
	public void close() {
	 ...
	 //}
	 mqttService.close(getClientHandle());
	}
	...
	public IMqttToken connect(MqttConnectOptions options, Object userContext,
		...
		//	connectOptions = PushUtil.getOptions();
		//}
		connectToken = token;

		/*
		 * 实际是否连接取决于服务状态。这个服务我们在这里绑定和启动。但是必须等onServiceConnected()（异步执行）方法运行
		 * 完成后才可以使用。所以,connection自身？？？
		 * 
		 * The actual connection dends on the service, which we start and bind
		...
		if (mqttService == null) { // First time - must bind to the service
			...
			serviceStartIntent.setClassName(myContext, PushUtil.SERVICE_NAME);
			...
			if (service == null) {
				...
				if (listener != null) {
					listener.onFailure(token, new RuntimeException(
							"cannot start service " + PushUtil.SERVICE_NAME));
				}
			}
	...
	private void doConnect() {
		/*if (getClientHandle() == null) {
			if(serverURI==null) {
				serverURI = PushUtil.BROKER_URL;
				clientId = PushUtil.getClientId(mqttService);
			}
			
			clientHandle = mqttService.getClient(serverURI, clientId,myContext.getApplicationInfo().packageName,persistence);
		}*/
		mqttService.setTraceEnabled(traceEnabled);
		mqttService.setTraceCallbackId(getClientHandle());
		
		String activityToken = storeToken(connectToken);
		try {
			//Notify.toast(mqttService, ""+clientHandle+"connectOptions"+connectOptions, Toast.LENGTH_LONG);
			//变量会被回收吧？ zwt
			//if(connectOptions==null) {
			//	connectOptions = PushUtil.getOptions();
			//}
			
			mqttService.connect(getClientHandle(), PushUtil.getOptions(), null,
					activityToken);
		}
	...
	public IMqttToken disconnect() throws MqttException {
		...
		mqttService.disconnect(getClientHandle(), null, activityToken);
		return token;
	}
	...
	public IMqttToken disconnect(long quiesceTimeout) throws MqttException {
		IMqttToken token = new MqttTokenAndroid(this, null,
				(IMqttActionListener) null);
		String activityToken = storeToken(token);
		mqttService.disconnect(getClientHandle(), quiesceTimeout, null,
				activityToken);
		return token;
	}
	...
	public IMqttDeliveryToken publish(String topic, byte[] payload, int qos,
		...
		String activityToken = storeToken(token);
		IMqttDeliveryToken internalToken = mqttService.publish(getClientHandle(),
	...
	public IMqttToken subscribe(String topic, int qos, Object userContext,
		...
		String activityToken = storeToken(token);
		mqttService.subscribe(getClientHandle(), topic, qos, null, activityToken);
		return token;
	}
	...
	public IMqttToken unsubscribe(String topic, Object userContext,
		...
		IMqttToken token = new MqttTokenAndroid(this, userContext,
				callback);
		String activityToken = storeToken(token);
		mqttService.unsubscribe(getClientHandle(), topic, null, activityToken);
		return token;
	}
	...
	public IMqttDeliveryToken[] getPendingDeliveryTokens() {
		return mqttService.getPendingDeliveryTokens(getClientHandle());
	}
	...
	public void onReceive(Context context, Intent intent) {
		...
		if ((handleFromIntent == null)
				|| (!handleFromIntent.equals(getClientHandle()))) {
			return;
		}
	...
	public boolean acknowledgeMessage(String messageId) {
		if (messageAck == Ack.MANUAL_ACK) {
			Status status = mqttService.acknowledgeMessageArrival(getClientHandle(), messageId);
		...
		if (callback != null) {
			...
			try {
				if (messageAck == Ack.AUTO_ACK) {
					...
					mqttService.acknowledgeMessageArrival(getClientHandle(), messageId);
				}
				else {
