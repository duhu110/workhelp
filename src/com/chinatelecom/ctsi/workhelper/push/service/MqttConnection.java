 ...
 */
package com.chinatelecom.ctsi.workhelper.push.service;
...
import com.chinatelecom.ctsi.workhelper.pushutil.Connection;
import com.chinatelecom.ctsi.workhelper.pushutil.Connections;
...
//import com.example.pushtest.mqtt.service.MessageStore.StoredMessage;
...
//import com.example.pushtest.util.push.Notify;
//import com.example.pushtest.util.push.PushUtil;

/**
 * <p>
 * MqttConnection holds a MqttAsyncClient {host,port,clientId} instance to
 * perform MQTT operations to MQTT broker.
 * </p>
 * <p>
...
class MqttConnection implements MqttCallback {
	...
	private String serverURI = PushUtil.BROKER_URL;
	...
	public String getClientId() {
		// return clientId;
		return PushUtil.getClientId(service);
	}
		...
		try {
			...
			if (myClient != null) {
				if (isConnecting) {
					...
					service.traceDebug(TAG, "Connect return:isConnecting:"
							+ isConnecting + ".disconnected:" + disconnected);
					return;
				} else if (!disconnected && isConnected()) {
					service.traceDebug(TAG,
					...
					myClient.connect(connectOptions, invocationContext,
							listener);
				}
			}
			...
			// if myClient is null, then create a new connection
				...
				myClient = new MqttAsyncClient(serverURI, getClientId(),
	...
	private void doAfterConnectSuccess(final Bundle resultBundle) {
		...
		// 提醒 数据库中存储的消息
		// zwt 不提示 deliverBacklog();
		setConnectingState(false);
		// String handle = PushUtil.getHandle(service.getApplicationContext());
		// Connection connection =
		// Connections.getInstance(service.getApplicationContext()).getConnection(handle);
	...
	private void handleException(final Bundle resultBundle, Exception e) {
		...
		resultBundle
				.putSerializable(MqttServiceConstants.CALLBACK_EXCEPTION, e);
	 ...
	 * have already purged(清除) any such messages from our messageStore.
	 */
	...
	public void connectionLost(Throwable why) {
		Notify.toast(service.getApplicationContext(), "connectionLost",
				Toast.LENGTH_SHORT);
		...
		// zwt add
		String handle = PushUtil.getHandle(service.getApplicationContext());
		Connection connection = Connections.getInstance(
				service.getApplicationContext()).getConnection(handle);
		connection
	...
	public void messageArrived(String topic, MqttMessage message)
		...
		service.callbackToActivity(clientHandle, Status.OK, resultBundle);
		// 去掉重复
		String preMessage = PushUtil.getPreMessage(service);
		// if(!"".equals(preMessage)) {
		// String preContent = new String(preMessage.getPayload());
		String content = new String(message.getPayload());
		if (content.equals(preMessage)) {
			Log.d("MqttCallbackHandlermmm", "重复" + content);
			return;
		}
		// }
		PushUtil.setPreMessage(service, content);
		mmmessageArrived(topic, message);
	}

	// private static MqttMessage preMessage;

	/**
	 * @see org.eclipse.paho.client.mqttv3.MqttCallback#messageArrived(String,
	 *      org.eclipse.paho.client.mqttv3.MqttMessage)
	 */
	private void mmmessageArrived(String topic, MqttMessage message) {

		String msg = new String(message.getPayload());

		PushMessage msg_object = JsonUtil.readJsonFromString(msg,
				PushMessage.class);
		if (msg_object != null) {
			if (msg_object.getType() == PushMessage.TYPE_NEW_TASK) {
				DutyInfo duty = msg_object.getInfo();
				if (duty.getExcutorId() == User.currentUser.id) {

					// if (DutyInfoDao.getInstance().getRecordById(
					// String.valueOf(duty.getId())) == null) {
					DutyInfoDao.getInstance().save(duty);// 入库

					if (DutyInfoHelper.newDutyListener != null) {
						DutyInfoHelper.newDutyListener.OnNewDuty(duty);
					}
					addRemind(duty);// 添加到时提醒
	...
	private void addRemind(DutyInfo info) {

		Intent intent = new Intent(AlarmService.Broadcast_alarm_add);
		intent.putExtra("id", String.valueOf(info.getId()));
		WHApplication.getInstance().sendBroadcast(intent);
	}
	...
	synchronized void reconnect() {
		if (isConnecting) {
			service.traceDebug(TAG,
					"The client is connecting. Reconnect return directly.");
			return;
		}
		...
		if (disconnected && !cleanSession) {
			...
			try {
				...
				service.traceError(TAG, "Cannot reconnect to remote server."
						+ e.getMessage());
				setConnectingState(false);
				handleException(resultBundle, e);
			}
